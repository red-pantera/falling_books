...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if the_user is not None and the_user.verify_password(login_form.password.data):
            ...
            flash(u'Login is successful! Welcome %s!' % the_user.name, 'success')
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'Registration is successful! Welcome %s!' % form.name.data, 'success')
        login_user(the_user)
        return redirect(request.args.get('next') or url_for('main.index'))
...
def change_password():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'The password is updated successfully!', 'info')
