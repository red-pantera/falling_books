...
def edit(book_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(book)
        db.session.commit()
        flash(u'Book materials have been saved!', 'success')
    ...
    return render_template("book_edit.html", form=form, book=book, title=u"Editing book materials")
...
def add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'Book %s has been added !' % new_book.title, 'success')
...
def delete(book_id):
    ...
    db.session.commit()
    flash(u'Successfully deleted the book, the user has been unable to view the book', 'info')
